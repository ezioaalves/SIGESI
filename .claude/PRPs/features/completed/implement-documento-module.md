...
- Entidade `Documento` com campos: id, numero, data, subject, honorifico, body, tipo (enum), portaria, assinante (Pessoa), interessado (Pessoa), destino (String), anexos (ManyToMany com Arquivo)
...
**Primary Systems Affected**: documentos (new module), arquivos (existing - referenced), pessoas (existing - referenced)
**Dependencies**: Nenhuma dependencia externa nova; usa Arquivo e Pessoa existentes
...
- No create: resolve FKs via services dependentes (PessoaService, ArquivoService)
...
- Mensagens de validacao em portugues: "Subject e obrigatorio", "Corpo e obrigatorio"
...
- Criar entidade Documento com todos os campos do diagrama (numero, data, subject, honorifico, body, tipo, portaria, assinante, interessado, destino, anexos)
- Relacionamentos: assinante (ManyToOne Pessoa), interessado (ManyToOne Pessoa), anexos (ManyToMany Arquivo). Destino agora e String simples.
...
- **IMPLEMENT**: Entidade JPA com os seguintes campos:
  ...
  - `numero`: String (opcional) - numero do documento
  - `data`: LocalDate, nullable=false, com @PrePersist para setar LocalDate.now() se null
  - `subject`: String, @NotBlank(message = "Subject e obrigatorio"), @Column(nullable = false)
  - `honorifico`: String (opcional) - tratamento honorifico
  ...
  - `portaria`: String (opcional) - referencia a portaria
  ...
  - `destino`: String (opcional) - destino como texto livre
...
- **IMPORTS**: jakarta.persistence.*, jakarta.validation.constraints.*, lombok.*, java.time.LocalDate, Pessoa, Arquivo
...
- **IMPLEMENT**: DTO para criacao com campos:
  ...
  - `subject`: String, @NotBlank(message = "Subject e obrigatorio")
  - `honorifico`: String (opcional)
  - `body`: String, @NotBlank(message = "Corpo e obrigatorio")
  - `tipo`: DocumentoTipo, @NotNull(message = "Tipo e obrigatorio")
  - `portaria`: String (opcional)
...
- **IMPLEMENT**: Interface MapStruct mapper com:
  - `@Mapping(target = "id", ignore = true)` + `@Mapping(target = "data", ignore = true)` + `@Mapping(target = "assinante", source = "assinanteId")` + `@Mapping(target = "interessado", source = "interessadoId")` + `@Mapping(target = "anexos", source = "anexoIds")` em `toEntity(DocumentoCreateDTO dto)`. O campo `destino` (String) mapeia automaticamente.
  - `toDto(Documento entity)` - mapeamento automatico
  - `toDtoList(List<Documento> entities)`
  - `updateFromDto(DocumentoUpdateDTO dto, @MappingTarget Documento documento)` com `@Mapping(target = "id", ignore = true)`, `@Mapping(target = "data", ignore = true)` e ignore para campos de relacionamento (assinante, interessado, anexos) - serao tratados manualmente no service. Campos escalares (numero, subject, honorifico, body, tipo, portaria, destino) sao tratados pelo mapper automaticamente.
  - Default methods: `mapAssinante(Long id)`, `mapInteressado(Long id)`, `mapAnexos(List<Long> ids)` - cada um cria entidade stub via Builder
...
- **IMPLEMENT**: Service com CRUD completo:
  ...
  - `createDocumento(DocumentoCreateDTO dto)`: resolve assinante via PessoaService.getPessoEntityById(), interessado via PessoaService.getPessoEntityById(), anexos via ArquivoService.getArquivoEntityById() em loop. Seta entidades no entity apos mapper.toEntity(). Campo destino e String simples, mapeado automaticamente.
  - `updateDocumento(Long id, DocumentoUpdateDTO dto)`: busca entidade, aplica mapper.updateFromDto() para campos escalares (numero, subject, honorifico, body, tipo, portaria, destino), resolve FKs de Pessoa e Arquivo manualmente se os IDs estiverem presentes no DTO (null check), salva e retorna DTO
...
- **IMPORTS**: PessoaService, ArquivoService, NotFoundException, Collectors
- **GOTCHA**: Javadoc obrigatorio: `/** Servico para Documento. */`. No updateDocumento, alem do mapper.updateFromDto(), tratar manualmente os campos de FK: se dto.getAssinanteId() != null, resolver via PessoaService e setar. Idem para interessadoId e anexoIds. Cuidado com o limite de 50 linhas por metodo e 3 returns max - pode ser necessario extrair metodo auxiliar para resolucao de FKs no update.
- **VALIDATE**: `mvn checkstyle:check -pl .`
...
- **IMPLEMENT**: Testes de validacao da entidade Documento:
  - Teste subject nulo lanca violacao
  - Teste subject vazio lanca violacao
  - Teste subject apenas espacos lanca violacao
  ...
  - Teste criacao documento valido sem violacoes (com todos os campos opcionais: numero, honorifico, portaria, destino)
...
- **IMPORTS**: @Mock para DocumentoRepository, DocumentoMapper, PessoaService, ArquivoService. @InjectMocks para DocumentoService.
...
- **IMPLEMENT**: Testes do controller:
  ...
  - POST / retorna 400 quando dados invalidos (subject vazio)
...
- Atualizar documento com PATCH parcial (apenas subject, apenas tipo, etc.)
- Atualizar relacionamentos individuais (apenas assinante, apenas interessado)
...
- [ ] Relacionamentos corretos: assinante/interessado (ManyToOne Pessoa), anexos (ManyToMany Arquivo). Destino e String simples.
...
4. **Update parcial (PATCH)**: O updateDocumento trata campos escalares (numero, subject, honorifico, body, tipo, portaria, destino) via mapper e relacionamentos (assinante, interessado, anexos) manualmente, permitindo atualizacao parcial de qualquer campo.
...
6. **Destino como String**: O diagrama atualizado mudou destino de `endereco {FK}` para `STRING`. Isso simplifica a implementacao - nao e mais necessario EnderecoService como dependencia. Destino e agora um campo de texto livre.

7. **Novos campos**: `numero` (numero do documento), `honorifico` (tratamento honorifico), `portaria` (referencia a portaria) sao todos campos String opcionais.

8. **Renomeacao title -> subject**: O campo foi renomeado de `title` para `subject` no diagrama atualizado.
